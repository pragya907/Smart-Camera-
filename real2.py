...
def play_alarm():
    try:
        ...
        if not pygame.mixer.get_busy(): # Play only if not already playing
             alarm_sound.play()
        # time.sleep(2) # Removing sleep to avoid blocking the loop
...
FRAME_SKIP = 10  # Process 1 in every 3 frames
DISPLAY_WIDTH = 800
DISPLAY_HEIGHT = 600
PROCESS_WIDTH = 320 # Smaller size for much faster AI processing
PROCESS_HEIGHT = 320
...
while True:
    ...
    # --- 1. Resize frame for display ---
    # We do this every frame. All drawing will be on this `display_frame`.
    display_frame = cv2.resize(frame, (DISPLAY_WIDTH, DISPLAY_HEIGHT), interpolation=cv2.INTER_AREA)
    camera.set(cv2.CAP_PROP_BUFFERSIZE, 1)
    # --- 2. Process only on skipped frames ---
    if frame_count % FRAME_SKIP == 0:
        ...
        for det in detections:
            ...
            color = (0, 255, 0)
            if confidence > 0.2 and label.lower() in ["knife", "gun", "firearm", "pistol", "revolver", "rifle"]:
                color = (0, 0, 255)
                current_threat = True 
